...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Hover effect: Change smiley face to chaotic emojis
    smileyFace.addEventListener('mouseover', () => {
        const randomEmoji = chaoticEmojis[Math.floor(Math.random() * chaoticEmojis.length)];
        smileyFace.textContent = randomEmoji;
    });

    smileyFace.addEventListener('mouseout', () => {
        smileyFace.textContent = "😊"; // Reset to default smiley
    });

    // Click anywhere to transition to the title screen
    clickToStartOverlay.addEventListener('click', () => {
        clickToStartOverlay.style.display = 'none'; // Hide overlay
        showTitleScreen(); // Transition to title screen
    });
});
